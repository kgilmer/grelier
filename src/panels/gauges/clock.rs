...
use crate::panels::gauges::gauge::{
    GaugeClick, GaugeClickAction, GaugeModel, GaugeValue, GaugeValueAttention,
};
...
fn clock_icon_for_time(hour: u32, minute: u32) -> svg::Handle {
  ...
  <circle cx="{CENTER}" cy="{CENTER}" r="{FACE_RADIUS}" fill="url(#grelierGaugeGrad)" mask="url(#clockHandCutout)"/>
</svg>"##
    );
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let _ = sender.try_send(GaugeModel {
                id: "clock",
                icon: Some(icon),
                value: Some(GaugeValue::Text(time_text)),
            ...
            let interval = if show_seconds { 1 } else { 60 };
